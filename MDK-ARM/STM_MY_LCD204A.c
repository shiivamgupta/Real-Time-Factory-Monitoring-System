...
//Private functions
//1) Enable EN pulse
static void LCD204A_EnablePulse(void)
{
	HAL_GPIO_WritePin(PORT_RS_and_E, PIN_E, GPIO_PIN_SET);
	LCD204A_TIM_MicorSecDelay(writeTimeConstant);
	HAL_GPIO_WritePin(PORT_RS_and_E, PIN_E, GPIO_PIN_RESET);
	LCD204A_TIM_MicorSecDelay(60);
}
//2) RS control
static void LCD204A_RS(bool state)
{
...
static void LCD204A_write(uint8_t byte)
{
...
static void LCD204A_TIM_Config(void)
{
...
static void LCD204A_TIM_MicorSecDelay(uint32_t uSecDelay)
{
...
//5) Write command
static void LCD204A_writeCommand(uint8_t command)
{
	//Set RS to 0
	LCD204A_RS(false);
	//Call low level write parallel function
	LCD204A_write(command);
}
//6) Write 8 bits data
static void LCD204A_writeData(uint8_t data)
{
	//Set RS to 1
	LCD204A_RS(true);
	//Call low level write parallel function
...
static void LCD204A_write4bitCommand(uint8_t nibble)
{
...
void LCD204A_Begin8BIT(GPIO_TypeDef* PORT_RS_E, uint16_t RS, uint16_t E, GPIO_TypeDef* PORT_LSBs0to3, uint16_t D0, uint16_t D1, uint16_t D2, uint16_t D3, GPIO_TypeDef* PORT_MSBs4to7, uint16_t D4, uint16_t D5, uint16_t D6, uint16_t D7)
{
	...
	LCD204A_writeCommand(LCD_FUNCTIONSET | LCD_FUNCTION_N | LCD_FUNCTION_DL);
	//4. Display control (Display ON, Cursor ON, blink cursor)
	LCD204A_writeCommand(LCD_DISPLAYCONTROL | LCD_DISPLAY_B | LCD_DISPLAY_C | LCD_DISPLAY_D);
	//5. Clear LCD and return home
	LCD204A_writeCommand(LCD_CLEARDISPLAY);
	HAL_Delay(2);
}
//2) LCD begin 4 bits function
void LCD204A_Begin4BIT(GPIO_TypeDef* PORT_RS_E, uint16_t RS, uint16_t E, GPIO_TypeDef* PORT_MSBs4to7, uint16_t D4, uint16_t D5, uint16_t D6, uint16_t D7)
{
	//Set GPIO Ports and Pins data
	PORT_RS_and_E = PORT_RS_E;
	PIN_RS = RS;
	...
	HAL_Delay(20);
	//2. Attentions sequence
	LCD204A_write4bitCommand(0x3);
	HAL_Delay(5);
	LCD204A_write4bitCommand(0x3);
	HAL_Delay(1);
	LCD204A_write4bitCommand(0x3);
	HAL_Delay(1);
	LCD204A_write4bitCommand(0x2);  //4 bit mode
	HAL_Delay(1);
	...
	LCD204A_writeCommand(LCD_FUNCTIONSET | LCD_FUNCTION_N);
	HAL_Delay(3);
}
//3) LCD print string
void LCD204A_print(char string[])
{
	for(uint8_t i=0;  i< 20 && string[i]!=NULL; i++)
	{
		LCD204A_writeData((uint8_t)string[i]);
	}
}


/********************************************/
void LCD240A_PrintChar(uint8_t Data){

	LCD204A_writeData(Data);
...
void LCD204A_setCursor(uint8_t row, uint8_t col)
{
	uint8_t maskData;
	maskData = (col-1)&0x0F;
	if(row==1)
...
void LCD204A_1stLine(void)
{
	LCD204A_setCursor(1,1);
}
void LCD204A_2ndLine(void)
{
	LCD204A_setCursor(2,1);
}

void LCD204A_3rdLine(void)
{
	LCD204A_setCursor(3,1);
}
void LCD204A_4thLine(void)
{
	LCD204A_setCursor(4,1);
}
//5) Enable two lines
void LCD204A_TwoLines(void)
{
	FunctionSet |= (0x08);
	LCD204A_writeCommand(FunctionSet);
}
void LCD204A_OneLine(void)
{
	FunctionSet &= ~(0x08);
	LCD204A_writeCommand(FunctionSet);
}
//6) Cursor ON/OFF
void LCD204A_noCursor(void)
{
	DisplayControl &= ~(0x02);
	LCD204A_writeCommand(DisplayControl);
}
void LCD204A_cursor(void)
{
	DisplayControl |= (0x02);
	LCD204A_writeCommand(DisplayControl);
}
//7) Clear display
void LCD204A_clear(void)
{
	LCD204A_writeCommand(LCD_CLEARDISPLAY);
	HAL_Delay(3);
}
//8) Blinking cursor
void LCD204A_noBlink(void)
{
	DisplayControl &= ~(0x01);
	LCD204A_writeCommand(DisplayControl);
}
void LCD204A_blink(void)
{
	DisplayControl |= 0x01;
	LCD204A_writeCommand(DisplayControl);
}
//9) Display ON/OFF
void LCD204A_noDisplay(void)
{
	DisplayControl &= ~(0x04);
	LCD204A_writeCommand(DisplayControl);
}
void LCD204A_display(void)
{
	DisplayControl |= (0x04);
	LCD204A_writeCommand(DisplayControl);
}
//10) Shift Display, right or left
void LCD204A_shiftToRight(uint8_t num)
{
	for(uint8_t i=0; i<num;i++)
	{
		LCD204A_writeCommand(0x1c);
	}
}
void LCD204A_shiftToLeft(uint8_t num)
{
	for(uint8_t i=0; i<num;i++)
	{
		LCD204A_writeCommand(0x18);
	}
}

//********** Print numbers to LCD **********//
//1. Integer
void LCD204A_PrintInt(int number)
{
	char numStr[16];
	sprintf(numStr,"%d", number);
	LCD204A_print(numStr);
}
//2. Float
void LCD204A_PrintFloat(float number, int decimalPoints)
{
	char numStr[16];
